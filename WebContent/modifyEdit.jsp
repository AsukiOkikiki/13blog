...
<body id="editBlog_bg">
	<%
		...
		if (obj == null) {
			...
			List<BlogInfo> list = BlogQuery.getBlogText(id);
			BlogInfo bi = list.get(0);
	%>
	...
	<form action="/Blog/test?op=modify&id=<%=bi.getId()%>" method="post">
		<div class="edit_text">
			<div>
				<textarea name="title" placeholder="请输入标题" class="edit_titlearea"><%=bi.getTitle()%></textarea>
			</div>
			<div>
				<textarea name="text" placeholder="这里是正文" class="edit_textarea"><%=bi.getText()%></textarea>
			</div>
			<div>
				...
				</select> &emsp; &emsp; 作者:<input name="author" value="<%=bi.getAuthor()%>"
					type="text">&emsp; &emsp;&emsp; &emsp;
